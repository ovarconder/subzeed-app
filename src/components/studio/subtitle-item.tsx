'use client';

import { useState, useRef, useEffect } from 'react';
import type { SubtitleEntry, TextSegment } from '@/lib/types';
import { textToSegments } from '@/lib/types';

interface SubtitleItemProps {
 sub: SubtitleEntry;
 index: number;
 isSelected: boolean;
 videoRef: React.RefObject<HTMLVideoElement | null>;
 onSelect: () => void;
 onUpdate: (updates: Partial<SubtitleEntry>) => void;
 onDelete: () => void;
}

/**
* SubtitleItem — แสดง 1 รายการใน sidebar
* - คลิกเพื่อเลือก
* - คลิกที่ข้อความเพื่อแก้ไข (inline edit)
* - Enter = บันทึก, Escape = ยกเลิก
* - รองรับการแสดง segments (หลายสี/สไตล์)
*/
export function SubtitleItem({
 sub,
 index,
 isSelected,
 videoRef,
 onSelect,
 onUpdate,
 onDelete,
}: SubtitleItemProps) {
 const [editing, setEditing] = useState(false);
 const [editText, setEditText] = useState(sub.text);
 const [editingTime, setEditingTime] = useState<'start' | 'end' | null>(null);
 const [editTimeValue, setEditTimeValue] = useState('');
 const inputRef = useRef<HTMLInputElement>(null);
 const timeInputRef = useRef<HTMLInputElement>(null);

 // Focus input เมื่อเริ่มแก้ไข
 useEffect(() => {
   if (editing && inputRef.current) {
     inputRef.current.focus();
     inputRef.current.select();
   }
 }, [editing]);

 useEffect(() => {
   if (editingTime && timeInputRef.current) {
     timeInputRef.current.focus();
     timeInputRef.current.select();
   }
 }, [editingTime]);

 // อัปเดต editText เมื่อ sub.text เปลี่ยนจากภายนอก (เช่น กดถอดความใหม่)
 useEffect(() => {
   if (!editing) setEditText(sub.text);
 }, [sub.text, editing]);

 const handleStartEdit = (e: React.MouseEvent) => {
   e.stopPropagation();
   setEditText(sub.text);
   setEditing(true);
 };

 const handleSaveText = () => {
   const trimmed = editText.trim();
   if (trimmed && trimmed !== sub.text) {
     // อัปเดตทั้ง text และ segments หลัก
     const segments = textToSegments(sub.id, trimmed);
     onUpdate({ text: trimmed, segments });
   }
   setEditing(false);
 };

 const handleKeyDown = (e: React.KeyboardEvent) => {
   if (e.key === 'Enter') {
     handleSaveText();
   } else if (e.key === 'Escape') {
     setEditText(sub.text);
     setEditing(false);
   }
 };

 // --- Time editing ---
 const formatTimeDisplay = (seconds: number) => {
   const m = Math.floor(seconds / 60);
   const s = (seconds % 60).toFixed(1);
   return `${m}:${s.padStart(4, '0')}`;
 };

 const parseTimeInput = (val: string): number | null => {
   const parts = val.split(':');
   if (parts.length === 2) {
     const mins = parseFloat(parts[0]);
     const secs = parseFloat(parts[1]);
     if (!isNaN(mins) && !isNaN(secs)) return mins * 60 + secs;
   } else if (parts.length === 1) {
     const secs = parseFloat(parts[0]);
     if (!isNaN(secs)) return secs;
   }
   return null;
 };

 const handleStartTimeEdit = (e: React.MouseEvent, type: 'start' | 'end') => {
   e.stopPropagation();
   setEditingTime(type);
   setEditTimeValue(formatTimeDisplay(type === 'start' ? sub.start : sub.end));
 };

 const handleSaveTime = () => {
   if (!editingTime) return;
   const val = parseTimeInput(editTimeValue);
   if (val !== null && val >= 0) {
     const rounded = Math.round(val * 10) / 10;
     if (editingTime === 'start') {
       onUpdate({ start: rounded });
     } else {
       onUpdate({ end: rounded });
     }
   }
   setEditingTime(null);
 };

 const handleTimeKeyDown = (e: React.KeyboardEvent) => {
   if (e.key === 'Enter') {
     handleSaveTime();
   } else if (e.key === 'Escape') {
     setEditingTime(null);
   }
 };

 // ─── ตรวจสอบ segments ──────────────────────────────
 const segments: TextSegment[] = sub.segments && sub.segments.length > 0
   ? sub.segments
   : textToSegments(sub.id, sub.text);

 return (
   <div
     className={`group p-3 cursor-pointer hover:bg-surface text-sm transition-colors ${
       isSelected ? 'bg-primary-light border-l-2 border-primary' : 'border-l-2 border-transparent'
     }`}
     onClick={onSelect}
   >
     {/* Header: # + timestamp */}
     <div className="flex justify-between text-xs text-text-secondary mb-1">
       <span className="font-medium">#{index + 1}</span>
       <div className="flex items-center gap-1">
         {editingTime === 'start' ? (
           <input
             ref={timeInputRef}
             type="text"
             value={editTimeValue}
             onChange={(e) => setEditTimeValue(e.target.value)}
             onBlur={handleSaveTime}
             onKeyDown={handleTimeKeyDown}
             className="w-14 rounded border border-primary px-1 py-0.5 text-[10px] text-center bg-white"
             onClick={(e) => e.stopPropagation()}
           />
         ) : (
           <span
             className="cursor-pointer hover:text-primary"
             onClick={(e) => handleStartTimeEdit(e, 'start')}
             title="คลิกแก้ไขเวลาเริ่ม"
           >
             {formatTimeDisplay(sub.start)}
           </span>
         )}
         <span className="text-text-disabled">→</span>
         {editingTime === 'end' ? (
           <input
             ref={timeInputRef}
             type="text"
             value={editTimeValue}
             onChange={(e) => setEditTimeValue(e.target.value)}
             onBlur={handleSaveTime}
             onKeyDown={handleTimeKeyDown}
             className="w-14 rounded border border-primary px-1 py-0.5 text-[10px] text-center bg-white"
             onClick={(e) => e.stopPropagation()}
           />
         ) : (
           <span
             className="cursor-pointer hover:text-primary"
             onClick={(e) => handleStartTimeEdit(e, 'end')}
             title="คลิกแก้ไขเวลาจบ"
           >
             {formatTimeDisplay(sub.end)}
           </span>
         )}
       </div>
     </div>

     {/* Text content — inline edit หรือแสดง segments */}
     {editing ? (
       <input
         ref={inputRef}
         type="text"
         value={editText}
         onChange={(e) => setEditText(e.target.value)}
         onBlur={handleSaveText}
         onKeyDown={handleKeyDown}
         className="w-full rounded border border-primary px-2 py-1 text-sm bg-white"
         onClick={(e) => e.stopPropagation()}
       />
     ) : (
       <div
         className="line-clamp-2 cursor-text hover:bg-white/50 rounded px-1 -mx-1 py-0.5 text-text-primary"
         onClick={handleStartEdit}
         title="คลิกเพื่อแก้ไขข้อความ"
       >
         {sub.text}
       </div>
     )}

     {/* แสดงไอคอนบอกว่ามีหลายสี */}
     {!editing && sub.segments && sub.segments.length > 1 && (
       <div className="text-[9px] text-text-secondary/60 mt-0.5 flex items-center gap-1">
         <span>🎨</span>
         <span>{sub.segments.length} ส่วน</span>
       </div>
     )}

     {/* Actions — แสดงเมื่อ hover หรือ selected */}
     {(isSelected || editing) && (
       <div className="flex gap-1 mt-1.5 justify-end">
         <button
           className="text-[10px] text-text-secondary hover:text-red-500 px-1.5 py-0.5 rounded hover:bg-red-50 transition-colors"
           onClick={(e) => {
             e.stopPropagation();
             if (videoRef.current) {
               videoRef.current.currentTime = Math.max(0, sub.start - 2);
               videoRef.current.play();
             }
           }}
           title="เล่นก่อนหน้านี้ 2 วินาที"
         >
           ◀ เล่นก่อน
         </button>
         <button
           className="text-[10px] text-text-secondary hover:text-red-500 px-1.5 py-0.5 rounded hover:bg-red-50 transition-colors"
           onClick={(e) => {
             e.stopPropagation();
             if (confirm('ลบซับไตเติลนี้?')) {
               onDelete();
             }
           }}
           title="ลบ"
         >
           🗑️
         </button>
       </div>
     )}
   </div>
 );
}
